//Fonction qui sert à inserer une nouvelle question à choix ouvert, 
//utilisation pour se faire d'un input de type text
...
//Fonction qui sert à inserer une nouvelle question à choix multiple, 
//utilisation pour se faire d'un input de type checkbox
...
//Affichage du type de question dans le document HTML 
// + du bouton envoyé les reponses pour obtenir le score
...
function score(questions){
	...
	for (let i=0;i<questions.length;i++)
	{
		//Selection du type de question en fonction de ce qui est écrit dans le JSON
		//type ouverte ou multiples
		switch (questions[i]['type'])
		{
			//Pour le cas des questions du type ouverte
			case 'ouverte':
			{
			...
			//Pour le cas des questions du type multiple
			case 'multiple':
			{
			// todo
				let bonne_reponse="";
				...
				//calcul fractionnaire du pourcentage de la bonne réponse
...
window.onload = function(){
	// Pour la methode 1 du choix de selection des fichiers pour le questionnaire (cf README)
	fetch("questions.json") // A modifier par le nom du fichier JSON a charger pour le cas d'un questionnaire à choix ouvert
	.then(response => response.json())
	.then(json => affiche(json.questions));
	/*fetch("questions2.json") // A modifier par le nom du fichier JSON a charger pour le cas d'un questionnaire à choix multiples
	.then(response => response.json())
	.then(json => affiche(json.questions));*/
